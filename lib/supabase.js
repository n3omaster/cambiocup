...
const saveGameScore = async (name, score, day, flagged = false, nonce = null, flagReason = null) => {
	const row = { name, score, day, flagged, nonce }
	const { data, error } = await supabase.from('game_scores').insert({ ...row, flag_reason: flagReason }).select()
