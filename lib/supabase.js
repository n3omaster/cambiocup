...
const getCoinData = async () => {
	const [cupHistory, mlcHistory, clasicaHistory, etecsaHistory, bandecprepagoHistory, gasHistory] = await Promise.all([
		fetchHistory(1),
		fetchHistory(2),
		fetchHistory(3),
		fetchHistory(4),
	...
	return { cupHistory, mlcHistory, clasicaHistory, etecsaHistory, bandecprepagoHistory, gasHistory }
}
...
const saveGameScore = async (name, score, day) => {
	const { data, error } = await supabase
		.from('game_scores')
		.insert({ name, score, day })
		.select()
	return { data: data?.[0], error }
}
...
const saveCoinData = async (cupValue, mlcValue, clasicaValue, etecsaValue, bandecprepagoValue, gasValue) => {
	...
	if (error || !data) return { cup: null, mlc: null, clasica: null, etecsa: null, bandecprepago: null, gas: null, error }
...
export { getCoinData, saveCoinData, saveOffer, getRecentOffers, getHistoricalData, saveGameScore, getTopScores, countScoresAbove, countGameRuns, supabase, COIN_IDS }
